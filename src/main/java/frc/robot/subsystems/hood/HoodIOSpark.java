...
public class HoodIOSpark implements HoodIO { // Outlines the HoodIOSpark class
  ...
  private final ConfigurableProfiledPIDController
      hoodController = // Creates a new PID controller type
      new ConfigurableProfiledPIDController(
              0.0, 0.1, 0.0, new Constraints(0, 0), "Hood Controller");

  private final RelativeEncoder hoodEncoder =
      motor.getEncoder(); // Creates RelativeEncoder and sets it to "hoodEncoder"
  ...
  public void updateInputs(HoodIOInputs inputs) { // method execution of updateInputs
    currentAngleDeg =
        Math.toDegrees(
            hoodEncoder.getPosition()); // Sets current angle var to be what its at using math
  ...
  public void setAngle(double angle) { // Command execution of setAngle()
    motor.set(
        MathUtil.clamp(
            hoodController.calculate(currentAngleDeg, angle),
            -1,
            1)); // Sets the motor to the new pos based on current pos
  }
}
